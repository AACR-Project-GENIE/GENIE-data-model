...
#' `dx_drug_end_or_lastadm_int_<n>`, and `dx_reg_start_int` columns to the
#' regimen (`reg`) table in `lst`, representing the number of days from the
#' cancer diagnosis (for the cancer in that row, identified by
#' `redcap_ca_seq`) to the start, end, and combined end-or-last-administered
#' date of each drug in the regimen, and to the regimen start overall.
#' Computed by subtracting `dob_ca_dx_days` from the corresponding
#' DOB-indexed `drugs_startdt_int_<n>`, `drugs_enddt_int_<n>`,
#' `drugs_drug_end_or_lastadm_int_<n>`, and `dob_reg_start_int` columns (the
#' end-or-lastadm columns produced upstream by
#' [derive_drug_end_or_lastadm_int()] and `dob_reg_start_int` by
#' [derive_reg_start_int()]). The month versions (`_mos`) are intentionally
#' not produced here.
#'
...
#'   `drugs_drug_end_or_lastadm_int_<n>`, and `dob_reg_start_int` columns.
#'
#' @returns `lst` with the new `dx_drug_start_int_<n>`, `dx_drug_end_int_<n>`,
#'   `dx_drug_end_or_lastadm_int_<n>`, and `dx_reg_start_int` columns added
#'   to `lst$reg`.
#' @export
#'
#' @examples
