  ...
  dat |>
    dplyr::mutate(
      ...
      .ca_clin_tnm_group = dplyr::case_when(
        ca_clin_t_stage %in% "TX" & ca_clin_n_stage %in% "N0" ~
          "Occult Carcinoma",
        ca_clin_t_stage %in% c("TpIS", "pIS") & ca_clin_n_stage %in% "N0" ~
          "Stage 0",
        (ca_clin_t1_det %in% c("T1a", "T1b") | ca_clin_t2_det %in% "T2a") &
        ...
        (ca_clin_t_stage %in% "T4" & ca_clin_n_stage %in% "N0") |
          (ca_clin_t_stage %in% c("T3", "T4") & ca_clin_n_stage %in% "N1") |
          ((ca_clin_t_stage %in%
            c("T3", "T4") |
      ...
      .ca_path_tnm_group = dplyr::case_when(
        ...
        ca_path_t_stage %in% "TX" & ca_path_n_stage %in% "N0" ~
          "Occult Carcinoma",
        (ca_path_t1_det %in% c("T1a", "T1b") | ca_path_t2_det %in% "T2a") &
        ...
        (ca_path_t_stage %in% "T4" & ca_path_n_stage %in% "N0") |
          (ca_path_t_stage %in% c("T3", "T4") & ca_path_n_stage %in% "N1") |
          ((ca_path_t_stage %in%
            c("T3", "T4") |
