...
#' @param d_name the variable used to make a decision about whether a variable is selected or not. Usually "p.value" (the default).
eval_beta_auc_pval <- function(
  true_beta,
  coef_dat,
  return_type = "auc",
  d_name = "p.value"
  ...
  if (return_type %in% "auc") {
    ...
    cli::cli_alert_danger(
      "Return type not 'auc' or 'plot' so returning the whole object"
    )
    return(roc)
  }
}
