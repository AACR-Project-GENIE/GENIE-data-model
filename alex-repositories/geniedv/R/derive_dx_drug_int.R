...
#' `dx_drug_end_or_lastadm_int_<n>`, `dx_reg_start_int`,
#' `dx_reg_end_any_int`, and `dx_reg_end_all_int` columns to the regimen
#' (`reg`) table in `lst`, representing the number of days from the cancer
#' diagnosis (for the cancer in that row, identified by `redcap_ca_seq`) to
#' the start, end, and combined end-or-last-administered date of each drug
#' in the regimen, and to the regimen start, end-of-any-drug, and
#' end-of-all-drugs overall. Computed by subtracting `dob_ca_dx_days` from
#' the corresponding DOB-indexed `drugs_startdt_int_<n>`,
#' `drugs_enddt_int_<n>`, `drugs_drug_end_or_lastadm_int_<n>`,
#' `dob_reg_start_int`, `dob_reg_end_any_int`, and `dob_reg_end_all_int`
#' columns (the end-or-lastadm columns produced upstream by
#' [derive_drug_end_or_lastadm_int()], `dob_reg_start_int` by
#' [derive_reg_start_int()], `dob_reg_end_any_int` by
#' [derive_reg_end_any_int()], and `dob_reg_end_all_int` by
#' [derive_reg_end_all_int()]). The month versions (`_mos`) are
...
#'   `drugs_drug_end_or_lastadm_int_<n>`, `dob_reg_start_int`,
#'   `dob_reg_end_any_int`, and `dob_reg_end_all_int` columns.
#'
#' @returns `lst` with the new `dx_drug_start_int_<n>`, `dx_drug_end_int_<n>`,
#'   `dx_drug_end_or_lastadm_int_<n>`, `dx_reg_start_int`,
#'   `dx_reg_end_any_int`, and `dx_reg_end_all_int` columns added to
#'   `lst$reg`.
#' @export
#'
#' @examples
...
derive_dx_drug_int <- function(lst) {
  ...
  lst[["reg"]] <- lst[["reg"]] |>
    ...
    dplyr::mutate(
      ...
      dx_reg_end_all_int = dob_reg_end_all_int - dob_ca_dx_days
    ) |>
