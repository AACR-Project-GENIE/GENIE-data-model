...
n80_cox_fdr <- read_wrap('gen_dat_one_n80_cox_uni_fdr.rds')
n500_cox_fdr <- read_wrap('gen_dat_one_n500_cox_uni_fdr.rds')
...
n80_cox_fdr %<>% mutate(n = 80)
n500_cox_fdr %<>% mutate(n = 500)
...
sim_sum_all %<>%
  mutate(
    avg_abs_bias_selected = if_else(
      is.nan(avg_abs_bias_selected),
      NA_real_,
      avg_abs_bias_selected
    ),
    avg_bias_selected = if_else(
      is.nan(avg_abs_bias_selected),
      NA_real_,
      avg_abs_bias_selected
    )
